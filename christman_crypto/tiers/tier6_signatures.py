...
class PQSigner:
    ...
    def keygen(self):
        return self.signer.generate_keypair()
...
class HybridSigner:
    ...
    def sign(self, message: bytes) -> bytes:
        ...
        if self.pq:
            _pq_pk = self.pq.keygen()  # keygen stores sk internally; returns pk
            pq_sig = self.pq.sign(None, message)  # sk=None, uses internal state
