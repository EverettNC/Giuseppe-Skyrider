...
class FusionEngine:
    ...
    def __init__(self):
        ...
        try:
            self.evo_brain = EvolutionaryAI(population_size=10, input_size=8, output_size=3, mutation_rate=0.1)
            self.evo_brain.load_fittest()
        except Exception as e:
            logger.error(f"EvolutionaryAI init failed in FusionEngine: {e}")
            self.evo_brain = None
            
        self.frustration_markers = ["not what i meant", "wrong", "stop", "no", "frustrated", "bad", "incorrect"]
    
    def get_persona_weights(self) -> Dict[str, float]:
        """
        Dynamic Mutation: Maps the winning architecture's outputs to Giuseppe's persona.
        Returns sass, hype, and action-confidence temperatures.
        """
        try:
            if not getattr(self, "evo_brain", None):
                raise ValueError("Evolutionary Engine not initialized")
                
            # Use shared latent state norm as baseline context vector
            latent_vector = list(self.z.values())[:8] if self.z else [0.5] * 8
            latent_vector = (latent_vector + [0.0] * 8)[:8]
            
            # Prediction outputs map to 3 persona traits (0.0 to 1.0)
            preds = self.evo_brain.population[0].predict(latent_vector)
            
            return {
                "sass": min(1.0, max(0.0, preds[0])),
                "hype": min(1.0, max(0.0, preds[1])),
                "action_confidence": min(1.0, max(0.0, preds[2])),
                "mode": "evolved"
            }
        except Exception as e:
            logger.error(f"Rule 6 Safety Check: Persona mapping failed. Defaulting to safe conservative block: {e}")
            return {
                "sass": 0.2,
                "hype": 0.3,
                "action_confidence": 0.1,  # Highly conservative
                "mode": "fallback_safe"
            }

    def evolve_understanding(self, interaction_data: List[float], feedback_score: float):
        """
        The Fitness Loop: called when the system receives a negative or positive interaction loop.
        Runs evolution in a background thread.
        """
        def evolve_task():
            try:
                if not getattr(self, "evo_brain", None):
                ...
                for ind in self.evo_brain.population:
                    # Multi-output prediction average distance to target feedback
                    preds = ind.predict(interaction_data)
                    avg_pred = sum(preds) / len(preds)
                    error = abs(avg_pred - feedback_score)
                    ind.fitness = max(0.0, 1.0 - error) * 100.0
                
                self.evo_brain.evolve_step()
                self.evo_brain.save_fittest()
                logger.info(f"Evolutionary understanding updated (Generation {self.evo_brain.generation})")
            except Exception as e:
                logger.error(f"Evolutionary Engine fallback. Failed to evolve understanding: {e}")
    ...
    def step(self, intent_text: str) -> Dict:
        ...
        # State tracking for evolutionary feedback
        lower_intent = intent_text.lower()
        is_frustrated = any(m in lower_intent for m in self.frustration_markers)
        feedback_score = 0.0 if is_frustrated else min(1.0, score)
        
        # Create an 8-dimension interaction context vector
        keys = list(chosen_c.keys())[:8]
        interaction_data = [chosen_c.get(k, 0.0) for k in keys]
        interaction_data = (interaction_data + [0.0] * 8)[:8]  # Pad to 8 dims
        
        if is_frustrated or score > 0.8:  # Positive reinforcement or correction
            self.evolve_understanding(interaction_data, feedback_score)
